...
function Message({ newMessage, setNewMessage, popUpMessage }) {
  return newMessage ? (
    <div className="absolute top-0 z-20 flex items-center justify-center w-full h-full text-black p-96 ">
      <div className="absolute w-full h-full bg-gray-500 opacity-40"></div>
      <div className="relative z-30 w-full p-10 bg-gray-200 items- rounded-3xl h-96">
        <button
          onClick={() => {
